...
public final class WrapperConnection implements Connection {
    ...
    /** остальные методы выполняем через Connection connect, кроме close() */
    @Override
    ...
    @Override
    public void close() throws SQLException { // Этот метод оставляем пустым, чтобы после каждого вызова Connection
         //ignore                             //в try-with-resources не закрывать соединение, а закрыть в конце
    }
